...
def _edit_section_content(ctx: Ctx, args: dict) -> str:
	...
	if mode == "find_replace":
		...
		if not find:
			return _("Provide the exact `find` text (and `replace`) for find_replace mode.")
		if replace is None:
			return _("Provide `replace` (may be empty to delete the matched text).")
		...
		if content is None:
			return _("Provide `content` (the full new markdown body) for replace mode.")
		new = content
	else:
		return _("`mode` must be 'replace' or 'find_replace'.")

	frappe.db.set_value("Source Section", name, "markdown", new, update_modified=False)
